# Contributing to Oracle E-Business Suite Browser Automation
...
- **Broken selectors**: Oracle E-Business Suite updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real Oracle E-Business Suite instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.ORACLE_EBS_URL}/path/to/section`);
...
When you find working selectors for Oracle E-Business Suite, document them:

```javascript
// Oracle E-Business Suite selectors (verified YYYY-MM-DD)
...
Oracle E-Business Suite updates its UI periodically. When selectors break:
