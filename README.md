# Oracle E-Business Suite Browser Automation
[![npm](https://img.shields.io/npm/v/@browser-automation-hub%2Foracle-ebs-browser-automation.svg)](https://www.npmjs.com/package/@browser-automation-hub/oracle-ebs-browser-automation)

> Automate Oracle E-Business Suite — the reliable way to interact with Oracle EBS programmatically, with or without an official API.
...
<!-- keywords: oracle ebs automation, oracle e-business suite automation, oracle forms automation, ebs browser automation, oracle erp automation, r12 automation -->

## What This Is

**Oracle E-Business Suite** (ERP) is notoriously difficult to automate via its official API — limited endpoints, complex authentication (Oracle SSO / LDAP / OAM), and browser-only workflows make traditional API integration a pain.

This project gives you a **complete browser automation scaffold** for Oracle E-Business Suite using Puppeteer (self-hosted, open source) or [Anchor Browser](https://anchorbrowser.io) (cloud, managed, production-ready).

This system uses **SSO** (Oracle SSO / LDAP / OAM). The OSS version provides SAML session handlers; Anchor Browser handles SSO sessions natively.

## Quick Start

```bash
git clone https://github.com/Browser-Automation-Hub/oracle-ebs-browser-automation.git
cd oracle-ebs-browser-automation
npm install
cp .env.example .env
...
- Oracle EBS on-prem without REST access
- Financial data extraction automation
...
- Oracle E-Business Suite account with appropriate permissions
...
```env
ORACLE_EBS_URL=https://your-ebs.example.com/OA_HTML/AppsLogin
ORACLE_EBS_USERNAME=your-username
ORACLE_EBS_PASSWORD=your-password
...
const { login_ebs } = require('./src/actions');

async function main() {
  const page = await createSession();
  const result = await login_ebs(page, { /* options */ });
  console.log(result);
}
...
2. **SAML SSO** (Oracle SSO / LDAP / OAM) — intercepts the SAML redirect and completes the IdP flow
3. **MFA / TOTP** (Oracle MFA / RSA) — generates TOTP codes via `otpauth` library
4. **Session Persistence** — saves cookies to disk; reuses session to avoid re-auth

### Handling Oracle MFA / RSA MFA
...
Oracle E-Business Suite API access requires additional licensing, certified partners, and months of integration work. Many batch operations are web-UI only.
...
- Handles Oracle MFA / RSA MFA automatically
...
> These CSS selectors were observed in Oracle E-Business Suite web interfaces. Enterprise applications update their UIs — verify against your specific instance and submit PRs when selectors break.

> 🔍 Selector reference not yet documented for Oracle E-Business Suite. [Contribute selectors via PR](https://github.com/Browser-Automation-Hub/oracle-ebs-browser-automation/pulls).
...
2. Document new selectors (they break when Oracle EBS updates its UI)
...
*Built with ❤️ for developers who need to automate Oracle E-Business Suite without wrestling with its API limitations. Powered by [Anchor Browser](https://anchorbrowser.io) for cloud-scale automation.*
