/**
 * anchor-cloud.js — AnchorBrowser cloud example for Oracle E-Business Suite
 *
...
const { login_ebs, navigate_responsibility } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting Oracle E-Business Suite automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_ebs(page, {});
      log('Action 1 result:', data1);

      const data2 = await navigate_responsibility(page, {});
